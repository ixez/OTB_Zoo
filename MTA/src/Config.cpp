...
// 	sz_backblock_x = 8;
// 	sz_backblock_y = 8;
// 	sz_backblock_x_step = 4;
// 	sz_backblock_y_step = 4;
// 	channel = 3;
// 	SetBackgroudNumbers();//	searchRadius = 600;
// }
	...
	int nSeed, int sRadius, double dsvmC, int svmBSize,std::string featName,std::string kernName, double dParam)
{
	...
	//resultsPath= seqpath +seqname+"_MRT.txt";
	resultsPath= seqname+"_MRT.txt";
	SelectRatioPath= "0_" +seqname+"_SR.txt";
	timePath= "0tr_" +seqname+".txt";

	seed = nSeed;
	searchRadius = sRadius;
	svmC = dsvmC;
	svmBudgetSize = svmBSize;
...
void Config::SetBackgroudNumbers()
{
	float tmp_x = (float)frameWidth/sz_backblock_x;
	float tmp_y = (float)frameHeight/sz_backblock_y;

	if (frameWidth%sz_backblock_x)
	{
		nNum_backgrounds_x = (int)(floorf(tmp_x)*2);
	}
	else
	{
		nNum_backgrounds_x = (int)(floorf(tmp_x)*2-1);
	}
	if (frameHeight%sz_backblock_y)
	{
		nNum_backgrounds_y = (int)(floorf(tmp_y)*2);
	}
	else
	{
		nNum_backgrounds_y = (int)(floorf(tmp_y)*2-1);
	}

	nNum_backgrounds = nNum_backgrounds_x*nNum_backgrounds_y;
}
