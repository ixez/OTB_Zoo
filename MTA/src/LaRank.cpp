...
//m_svs에 저장되어 있는 Support Vector들을 이용하여 Eval
...
void LaRank::Eval(const MultiSample& sample, std::vector<double>& results)
{
	...
	results.resize(sample.GetRects().size());
...
// 	{
// 		#pragma omp for
// 		for (int i = 0; i < (int)sample.GetRects().size(); ++i)
// 		{
...
// 	}
	...
	for (int i = 0; i < (int)sample.GetRects().size(); ++i)
	{
		FloatRect y(sample.GetRects()[i]);
		y.Translate(-centre.XMin(), -centre.YMin());
		Eigen::VectorXd tmpFeat;
		tmpFeat = const_cast<Features&>(m_features).Eval(sample.GetSample(i));
		results[i] = Evaluate(tmpFeat, y);		
	}
	//////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////
//Copy LaRank
void LaRank::CopyTo(LaRank& targetLaRank)
{
	targetLaRank.m_C = m_C;
	targetLaRank.m_K = m_K;
	if (DebugMode==1)
	{
		m_debugImage.copyTo(targetLaRank.m_debugImage);
	}
	targetLaRank.m_sps.clear();
	targetLaRank.m_svs.clear();
	targetLaRank.m_sps.resize(m_sps.size());
	for (int nSP = 0;nSP <m_sps.size();nSP++)
	{
		SupportPattern* sp = new SupportPattern;
		
		sp->x.resize(m_sps[nSP]->x.size());
		copy(m_sps[nSP]->x.begin(), m_sps[nSP]->x.end(), sp->x.begin());

		sp->yv.resize(m_sps[nSP]->yv.size());
		copy(m_sps[nSP]->yv.begin(), m_sps[nSP]->yv.end(), sp->yv.begin());
		
		if (DebugMode==1)
		{
			sp->images.resize(m_sps[nSP]->images.size());
			copy(m_sps[nSP]->images.begin(), m_sps[nSP]->images.end(), sp->images.begin());
		}

		sp->y =  m_sps[nSP]->y;
		sp->refCount =  m_sps[nSP]->refCount;

		targetLaRank.m_sps[nSP]= sp;
	}
	

	for (int nSV = 0;nSV <m_svs.size();nSV++)
	{
		SupportVector* sv = new SupportVector;
		sv->y = m_svs[nSV]->y;
		sv->b = m_svs[nSV]->b;
		...
		if (DebugMode==1)
		{
			m_svs[nSV]->image.copyTo(sv->image);		
		}
		
		//////////////////////////////////////////////////////////////////////////
// 		SupportPattern* sp = new SupportPattern;
// 
// 		sp->x.resize(m_svs[nSV]->x->x.size());
// 		copy(m_svs[nSV]->x->x.begin(), m_svs[nSV]->x->x.end(), sp->x.begin());
// 
// 		sp->yv.resize(m_svs[nSV]->x->yv.size());
// 		copy(m_svs[nSV]->x->yv.begin(), m_svs[nSV]->x->yv.end(), m_svs[nSV]->x->yv.begin());
// 
// 		if (DebugMode==1)
// 		{
// 			sp->images.resize(m_svs[nSV]->x->images.size());
// 			copy(m_svs[nSV]->x->images.begin(), m_svs[nSV]->x->images.end(), sp->images.begin());
// 		}
// 
// 		sp->y =  m_svs[nSV]->x->y;
// 		sp->refCount =  m_svs[nSV]->x->refCount;
		for (int nSP=0;nSP<m_sps.size();nSP++)
		{
			if (m_svs[nSV]->x==m_sps[nSP])
			{
				sv->x = targetLaRank.m_sps[nSP];
				break;
			}			
		}
		...
		targetLaRank.m_svs.push_back(sv);
	}	
}
//////////////////////////////////////////////////////////////////////////