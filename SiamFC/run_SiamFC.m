...
net_z = load_pretrained(p.net_path, p.gpus);
net_x = load_pretrained(p.net_path, []);

imgFiles = seq.s_frames;

initstate = seq.init_rect;
targetPosition = [initstate(2)+initstate(4)/2 initstate(1)+initstate(3)/2];
targetSize = [initstate(4) initstate(3)];
...
for i = startFrame:nImgs
    ...
    results.res = [results.res; rectPosition];
end
